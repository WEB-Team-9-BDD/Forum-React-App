  ...
  return (
    <>
      <header>
        ...
        <NavLink className="header-link" to="/post-create">
          Create Post
        </NavLink>
        {userData && userData.isAdmin ? (
          <NavLink className="header-link" to="/admin-powers">
            Admin Powers
          </NavLink>
        ) : undefined}
        {!user ? (
          ...
          <>
            <div className="header-username">
              {`${userData?.username[0].toUpperCase()}${userData?.username.slice(
                1
              )}'s room`}
            </div>
          </>
        ...
        {user && (
          <>
            <div className="profile-view" onClick={toggleProfile}>
              {!user.photoURL ? (
                <CgProfile className="header-profile-pic" />
              ) : (
                <img
                  src={`${user.photoURL}`}
                  alt="avatar"
            ...
            {visible && (
              <div className="header-profile">
                ...
                <div className="header-profile-content">
                  <NavLink to="/user-profile" onClick={toggleProfile}>
                    <CgProfile className="profile-icons" />
                    My room
                  </NavLink>
                  <NavLink to="/update-profile" onClick={toggleProfile}>
                    <MdOutlineSettings className="profile-icons" />
                    Settings
                  </NavLink>
                  <div className="logout" onClick={logout}>
                    {' '}
                    <FaPowerOff className="profile-icons" />
                    Log out
                  </div>
                </div>
              </div>
